...
outlier_scores = LocalOutlierFactor(n_neighbors=30, contamination=0.1).fit_predict(
    adata.obsm["X_umap"]
)
    ...
    (
        ...
        & (
            np.sum(
                np.array(
                    [
                        (adata[:, "Rbfox3"].to_df() > 0).stack().values,
                        (adata[:, "Dlx5"].to_df() > 0).stack().values,
                        (adata[:, "Elavl4"].to_df() > 0).stack().values,
                        (adata[:, "Stmn2"].to_df() > 0).stack().values,
                        (adata[:, "Snap25"].to_df() > 0).stack().values,
                        (adata[:, "Th"].to_df() > 0).stack().values,
                        (adata[:, "Slc17a6"].to_df() > 0).stack().values,
                        (adata[:, "Gad1"].to_df() > 0).stack().values,
                        (adata[:, "Gad2"].to_df() > 0).stack().values,
                        (adata[:, "Npy"].to_df() > 0).stack().values,
                        (adata[:, "Agrp"].to_df() > 0).stack().values,
                        (adata[:, "Crh"].to_df() > 0).stack().values,
                        (adata[:, "Trh"].to_df() > 0).stack().values,
                        (adata[:, "Avp"].to_df() > 0).stack().values,
                        (adata[:, "Pomc"].to_df() > 0).stack().values,
                        (adata[:, "Hcrt"].to_df() > 0).stack().values,
                        (adata[:, "Oxt"].to_df() > 0).stack().values,
                        (adata[:, "Cxcl14"].to_df() > 0).stack().values,
                        (adata[:, "Cxcl1"].to_df() > 0).stack().values,
                        (adata[:, "Cxcl2"].to_df() > 0).stack().values,
                        (adata[:, "Foxj1"].to_df() > 0).stack().values,
                        (adata[:, "Vim"].to_df() > 0).stack().values,
                        (adata[:, "Nes"].to_df() > 0).stack().values,
                        (adata[:, "Enkur"].to_df() > 0).stack().values,
                        (adata[:, "Foxj1"].to_df() > 0).stack().values,
                        (adata[:, "Kif6"].to_df() > 0).stack().values,
                        (adata[:, "Kif9"].to_df() > 0).stack().values,
                        (adata[:, "Hydin"].to_df() > 0).stack().values,
                        (adata[:, "Mog"].to_df() > 0).stack().values,
                        (adata[:, "Plp1"].to_df() > 0).stack().values,
                        (adata[:, "Cnp"].to_df() > 0).stack().values,
                        (adata[:, "Mag"].to_df() > 0).stack().values,
                        (adata[:, "Opalin"].to_df() > 0).stack().values,
                        (adata[:, "Sox10"].to_df() > 0).stack().values,
                        (adata[:, "Olig1"].to_df() > 0).stack().values,
                        (adata[:, "Olig2"].to_df() > 0).stack().values,
                        (adata[:, "Aif1"].to_df() > 0).stack().values,
                        (adata[:, "Itgam"].to_df() > 0).stack().values,
                        (adata[:, "Ptprc"].to_df() > 0).stack().values,
                        (adata[:, "Fcrls"].to_df() > 0).stack().values,
                        (adata[:, "Pdgfra"].to_df() > 0).stack().values,
                        (adata[:, "Pdgfrb"].to_df() > 0).stack().values,
                        (adata[:, "Gpr17"].to_df() > 0).stack().values,
                        (adata[:, "Ugt8a"].to_df() > 0).stack().values,
                        (adata[:, "Sema3c"].to_df() > 0).stack().values,
                        (adata[:, "Sema4d"].to_df() > 0).stack().values,
                        (adata[:, "Sema4f"].to_df() > 0).stack().values,
                        (adata[:, "Gpr37"].to_df() > 0).stack().values,
                        (adata[:, "Cspg4"].to_df() > 0).stack().values,
                        (adata[:, "Lingo1"].to_df() > 0).stack().values,
                        (adata[:, "Rgs5"].to_df() > 0).stack().values,
                        (adata[:, "Des"].to_df() > 0).stack().values,
                        (adata[:, "Acta2"].to_df() > 0).stack().values,
                        (adata[:, "Cd248"].to_df() > 0).stack().values,
                        (adata[:, "Myh11"].to_df() > 0).stack().values,
                        (adata[:, "Cdh5"].to_df() > 0).stack().values,
                        (adata[:, "Fgf10"].to_df() > 0).stack().values,
                        (adata[:, "Rax"].to_df() > 0).stack().values,
                        (adata[:, "Tbx3"].to_df() > 0).stack().values,
                    ]
                ),
                axis=0,
            )
