---
title: "CellChat signalling analysis of Insr positive vs. Insr negative astrocytes"
...
lopez_norm <-
  read_csv(here(
    data_dir,
    "signalling", "lopez",
    "pvn_astro-with_npy_pomc-liana-res-cellchatdb.csv"
  )
)
...
lopez_csds <-
  read_csv(here(
    data_dir,
    "signalling", "lopez",
    "chronic-stress-pvn_astro-with_npy_pomc-liana-res-cellchatdb.csv"
  )
)
```
...
```{r}
pr <- ggplot(df, aes(x = pairs, y = neg_log10_specificity_rank, color = as.factor(control), size = neg_log10_magnitude_rank)) +
