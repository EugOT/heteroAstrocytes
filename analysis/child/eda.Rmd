...
cell_bender_merged <-
  Read_CellBender_h5_Multi_Directory(
    ...
    custom_name = sprintf("_output_FPR_%s_filtered.h5", cb_fpr),
...
```{r normalise-raw, include=FALSE, echo=FALSE}
combined_srt <- NormalizeData(combined_srt, assay = "RAW")
```

```{r dual-assay-genes, results = "hide"}
src_list <- map(genes.embed, function(gene) {
    src <- c(
      "### {{gene}} {.unnumbered}",
      "```{r dual-assay-{{gene}}}",
      "FeaturePlot_DualAssay(",
        ...
        "assay1 = 'RNA', assay2 = 'RAW',",
        "colors_use = combined_srt@misc$expr_Colour_Pal,",
        "pt.size = 4, order = TRUE, alpha_na_exp = 0.1,",
        " alpha_exp = 0.45) &",
